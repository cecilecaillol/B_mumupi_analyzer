    ...
    pfToken_(consumes<pat::PackedCandidateCollection>(iConfig.getParameter<edm::InputTag>("pfCands")))
{
   ...
   tr->Branch("trkKink_1", &trkKink_1, "trkKink_1/F");
   tr->Branch("segmentCompatibility_1", &segmentCompatibility_1, "segmentCompatibility_1/F");
   tr->Branch("softMvaValue_1", &softMvaValue_1, "softMvaValue_1/F");
   tr->Branch("validFraction_2", &validFraction_2, "validFraction_2/F");
   tr->Branch("normalizedChi2_2", &normalizedChi2_2, "normalizedChi2_2/F");
   tr->Branch("chi2LocalPosition_2", &chi2LocalPosition_2, "chi2LocalPosition_2/F");
   tr->Branch("trkKink_2", &trkKink_2, "trkKink_2/F");
   tr->Branch("segmentCompatibility_2", &segmentCompatibility_2, "segmentCompatibility_2/F");
   tr->Branch("softMvaValue_2", &softMvaValue_2, "softMvaValue_2/F");
...
miniaod_ntuplizer::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
   ...
   if (muon_indices.size()>=2 and pion_indices.size()>=1){
	  ...
	  for (unsigned int i_pi=0; i_pi<pion_indices.size(); ++i_pi){
	    ...
	    if (pion.hasTrackDetails() && !muon1.innerTrack().isNull() && !muon2.innerTrack().isNull()){ // all tracks exist 
      ...
      validFraction_1=mymu1.innerTrack()->validFraction();
      validFraction_2=mymu2.innerTrack()->validFraction();
      normalizedChi2_1=mymu1.globalTrack()->normalizedChi2();
      normalizedChi2_2=mymu2.globalTrack()->normalizedChi2();
      chi2LocalPosition_1=mymu1.combinedQuality().chi2LocalPosition;
      chi2LocalPosition_2=mymu2.combinedQuality().chi2LocalPosition;
      trkKink_1=mymu1.combinedQuality().trkKink;
      trkKink_2=mymu2.combinedQuality().trkKink;
      segmentCompatibility_1=muon::segmentCompatibility(mymu1);
      segmentCompatibility_2=muon::segmentCompatibility(mymu2);
      softMvaValue_1=mymu1.softMvaValue();
      softMvaValue_2=mymu2.softMvaValue();
      ...
      //std::cout<<"Chosen: ("<<mymu1.pt()<<" "<<mymu2.pt()<<" "<<mypion.pt()<<")"<<std::endl;
